...
def output_paths(output_dir: Path, batch_date: date, digest_type: str) -> dict[str, Path]:
    stem = f"{batch_date.isoformat()}-{digest_type}-beginner"
    return {
...
def _digest_type_rules(digest_type: str) -> str:
...
- Do not write reading instructions like 보세요. State the lens as article copy.
...
- Main items must use these lenses: "한마디로 무슨 변화인가", "왜 이 문제가 있었나", "이번 방법은 무엇을 덜 필요하게 하나", "헷갈리지 말 것".
...
- what_changed must answer which burden is reduced: cost, access, manual work, data, execution, memory, time, or infrastructure.
...
def _schema_block(digest_type: str) -> str:
    if digest_type == "business":
        return """
{
...
def build_beginner_prompt(pair: DigestPair) -> str:
    source = {
...
- Keep skim_items light: each skim_items.why_skim must be 35 Korean words or fewer.
...
- Never copy schema labels as field content. Values such as "왜 이 문제가 있었나",
  "이번 방법은 무엇을 덜 필요하게 하나", or "헷갈리지 말 것" are section labels,
...
{_digest_type_rules(pair.digest_type)}

Return strict JSON with this shape:
{_schema_block(pair.digest_type)}
...
- Each skim_items.why_skim must be 35 Korean words or fewer.
...
def validate_beginner_payload(payload: dict[str, Any], digest_type: str) -> None:
    ...
    for field_name, value in reader_facing_fields:
        if isinstance(value, str) and PROJECT_OR_METHOD_FIRST_RE.search(value):
            raise ValueError(
    ...
    for index, item in enumerate(main_items, start=1):
        ...
        if digest_type == "research":
            ...
            if not any(cue in what_changed for cue in RESEARCH_WHAT_CHANGED_BURDEN_CUES):
                raise ValueError(
                    f"research main_items[{index}].what_changed must explain which burden is reduced"
                )


def build_revision_prompt(payload: dict[str, Any], validation_error: str) -> str:
...
- Each skim_items.why_skim must be 35 Korean words or fewer.
- Research main item body may use at most 2 technical method terms before dont_confuse or next_read.
- Research what_changed must explain which burden is reduced: cost, access, manual work, data, execution, memory, time, or infrastructure.
...
def render_article_markdown(payload: dict[str, Any], digest_type: str) -> str:
    ...
    for index, item in enumerate(main_items, start=1):
        ...
        if item.get("what_changed"):
            lines.extend(["**이번 방법은 무엇을 덜 필요하게 하나**", "", str(item["what_changed"]), ""])
...
def render_markdown(payload: dict[str, Any], digest_type: str, batch_date: date) -> str:
    type_title = digest_type.title()
    lines = [
        f"# {batch_date.isoformat()} {type_title} Beginner Preview",
        "",
        "> Prototype only. Source: existing news_posts rows. No DB write.",
        "",
        render_article_markdown(payload, digest_type).rstrip(),
    ]
    return "\n".join(lines).rstrip() + "\n"
...
def load_beginner_artifact(path: Path) -> dict[str, Any]:
    ...
    validate_beginner_payload(payload, digest_type)
    return artifact
...
def build_beginner_backfill_update(
    ...
    *,
    ...
    return ko_slug, {
        "content_beginner": render_article_markdown(payload, digest_type),
...
def apply_beginner_backfill_file(
    ...
    *,
    ...
    if not existing.data:
        raise RuntimeError(f"Missing target news_posts row for {ko_slug}")
...
async def generate_beginner_preview(
    ...
    *,
    ...
    prompt = build_beginner_prompt(pair)
    messages = [
        {
            "role": "system",
            "content": (
                "You are a careful Korean news editor. Return JSON only. "
    ...
    response = await client.chat.completions.create(
        **build_completion_kwargs(
            ...
            verbosity="low",
            prompt_cache_key=f"beginner-news-prototype:{pair.digest_type}",
        )
    )
    ...
    try:
        validate_beginner_payload(payload, pair.digest_type)
    except ValueError as exc:
        revision_messages = messages + [
            {"role": "assistant", "content": raw},
            {"role": "user", "content": build_revision_prompt(payload, str(exc))},
        ]
        revision_response = await client.chat.completions.create(
            **build_completion_kwargs(
                ...
                service_tier=service_tier,
                verbosity="low",
                prompt_cache_key=f"beginner-news-prototype-revision:{pair.digest_type}",
            )
        )
...
def write_prompt_file(pair: DigestPair, output_dir: Path) -> Path:
    output_dir.mkdir(parents=True, exist_ok=True)
    paths = output_paths(output_dir, pair.batch_date, pair.digest_type)
    paths["prompt"].write_text(build_beginner_prompt(pair), encoding="utf-8")
    return paths["prompt"]
...
def write_preview_files(
    ...
    paths["markdown"].write_text(
        render_markdown(payload, pair.digest_type, pair.batch_date),
        encoding="utf-8",
    )
    paths["prompt"].write_text(build_beginner_prompt(pair), encoding="utf-8")
    return paths
...
async def main() -> None:
    ...
    parser.add_argument(
        "--apply-backfill",
        action="store_true",
        help="Write an existing local beginner JSON artifact into the Korean news_posts row.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    for digest_type in digest_types:
        if args.apply_backfill:
            artifact_path = output_paths(args.output_dir, args.date, digest_type)["json"]
        ...
        prompt_path = write_prompt_file(pair, args.output_dir)
        print(f"{digest_type}: wrote prompt {prompt_path}")
        ...
        paths = write_preview_files(pair, payload, usage, args.output_dir)
        cost = usage.get("cost_usd")
        cost_text = f"${cost:.4f}" if isinstance(cost, (int, float)) else "unknown cost"
        print(
            f"{digest_type}: wrote {paths['markdown']} and {paths['json']} "
