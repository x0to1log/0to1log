...
function buildBusinessSaveBody({ learnerEditor, expertEditor }) {
  const saveBody = {};
  if (learnerEditor) {
...
{
  const body = buildBusinessSaveBody({ learnerEditor: null, expertEditor: null });
...
function buildBackendRow({ content_learner, content_expert }) {
  const row = {};
...
{
  const row = buildBackendRow({ content_expert: 'real content' });
