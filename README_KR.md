...
AI 파이프라인은 26일 동안 8번의 버전을 거쳤다. v1은 5일간 세 가지 아키텍처 결함을 체계적으로 발견했고, 그 결과 v2는 1일, v3는 반나절 만에 완성되었다. 8회의 프롬프트 반복으로 품질 점수를 76에서 95로 끌어올렸고, v8에서는 지시를 *제거*하는 것이 추가하는 것보다 나을 수 있다는 것을 발견했다. 아래 모든 수치는 프로덕션 DB에서 측정한 실측 데이터이다.
...
근본 원인 발견, 핵심 의사결정, 정량적 결과를 포함한 전체 기록은 [개발 여정](./docs/portfolio/pipeline-journey.ko.md)에 있다.
