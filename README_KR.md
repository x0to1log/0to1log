...
AI 파이프라인은 26일 동안 8번의 버전을 거쳤다. v1은 5일 만에 실패했다 — 아키텍처가 잘못되었고, 패치로는 해결할 수 없었다. 전체를 삭제하고 다시 만들자 v2는 1일, v3는 반나절 만에 완성되었다. 8회의 프롬프트 반복으로 품질 점수를 76에서 95로 끌어올렸고, v8에서는 지시를 *제거*하는 것이 추가하는 것보다 나을 수 있다는 것을 발견했다. 아래 모든 수치는 프로덕션 DB에서 측정한 실측 데이터이다.

| 지표 (실측) | v2–v4 | v5–v6 | v7–v8 |
|---|---|---|---|
| **평균 비용/run** | $0.35 | $0.40 | $0.48 |
| **Research Expert citation** | 1.8 | 12.9 | 16.8 |
| **Business Expert citation** | 2.7 | 13.9 | 14.2 |
| **커버 뉴스 건수** | 1.3–2.7 | 3.6–4.6 | 4.5–5.0 |
| **수집 소스** | 1 | 4 | 6 |

실패 원인, 핵심 의사결정, 정량적 결과를 포함한 전체 기록은 [개발 여정](./docs/portfolio/pipeline-journey.ko.md)에 있다.
...
- **기초**: 비유와 예시로 쉽게 설명
- **심화**: 벤치마크, 아키텍처 상세, 참조 링크 포함
- **자동 인라인 연결**: 뉴스 속 용어를 클릭하면 팝업으로 정의 확인
- **품질 파이프라인**: Tavily 검색 컨텍스트 + 유형 분류 + Self-critique (75점 미만 시 재생성) + 품질 점수
