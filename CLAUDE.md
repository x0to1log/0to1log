...
0. **계획 먼저.** 사소한 코드 수정(오타, 1줄 변경) 또는 vault 문서 동기화를 제외한 모든 작업은 구현 전에 계획을 세운다. Plan mode 또는 vault 계획 문서를 사용한다.
...
- 완료된 설계/계획 문서는 `vault/90-Archive/YYYY-MM/plans-completed/` 또는 `plans-archive/`로 이동 (읽기 전용).
- 활성 계획만 `vault/09-Implementation/plans/`에 유지 (ACTIVE_SPRINT.md와 현재 Phase 관련 파일들).
...
- **0to1-prompt-audit**: 프롬프트 파일 감사 (P0/P1/P2 품질 기준)
  - 대상: `backend/services/agents/prompts_advisor.py`, `prompts_news_pipeline.py`, `prompts_handbook_types.py`
  - 기준: `vault/09-Implementation/plans/2026-03-18-prompt-audit-fixes.md` (live 참조)
  - 사용: `/prompt-audit [advisor|news|handbook]` (인자 없으면 3개 파일 모두 감사)

- **0to1-sprint-sync**: 스프린트 시작/종료 동기화
  - Start mode: ACTIVE_SPRINT.md의 진행중/최종검증/남은태스크 현황 리포트 + 다음 task 추천
  - End mode: completed tasks 마크 + vault notes 식별 + 커밋 제안
  - 참조: `vault/09-Implementation/plans/ACTIVE_SPRINT.md`
