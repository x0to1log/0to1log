...
| # | 워크스트림 | 규모 | 의존성 |
|---|---|---|---|
| **A** | Dead code 완전 제거 (PromptGuideItems 포함) | 7 파일 (backend 4 + frontend 1 + vault 2) | 독립 |
...
**Backend (4)** — 실제 수정 파일 (advisor.py는 grep 결과 참조 없음으로 drop)
| 파일 | 변경 |
|---|---|
| `backend/models/common.py` | `PromptGuideItems` 클래스 완전 삭제 (현재 line 12-17) |
| `backend/models/posts.py` | import 제거 (line 5) + `Optional[PromptGuideItems]` → `Optional[dict]` 2군데 교체 (line 45, 69) |
| `backend/services/agents/prompts_advisor.py` | `_GENERATE_GUIDE_ITEMS` 딕셔너리의 4개 digest_type(ai-news/study/career/project)에서 각각 `one_liner` / `action_item` / `critical_gotcha` / `rotating_item` 키 제거. dict 범위 line 49-73 |
| `backend/openapi.json` | **3곳 정리 필요**: (1) `PromptGuideItems` 컴포넌트 스키마 제거 (line 941-971), (2) 두 곳의 `$ref: #/components/schemas/PromptGuideItems` 참조 업데이트 (line 588, 905) — inline schema 또는 `{"type":"object"}` 로 교체, (3) component 내부 `required: [one_liner, action_item, critical_gotcha, rotating_item, quiz_poll]` 배열 (line 965-970) — 삭제와 함께 사라짐. 가능하면 FastAPI 재생성이 가장 안전 |

**검증된 non-action**: `backend/services/agents/advisor.py`는 4개 필드·PromptGuideItems·guide_items에 대한 참조 **전무** (spec reviewer grep 확인). 수정 불필요.

**Frontend (1)** — 실제 수정 파일
| 파일 | 변경 |
|---|---|
| `frontend/src/components/newsprint/NewsprintArticleLayout.astro` | `interface GuideItems`에서 **필드명 기준** 제거: `one_liner` / `action_item` / `critical_gotcha` / `rotating_item` 4개. **`quiz_poll` / `weekly_quiz_expert` / `weekly_quiz_learner`는 유지** (실수 주의). `hasGuideItems` 조건에서 제거된 필드 참조 빼기. 4개 dead render 블록 제거 (필드명으로 식별) |

**검증된 non-action**: `frontend/src/pages/admin/edit/[slug].astro` — dead 필드에 대한 `data-apply="one_liner"` 식 버튼 **없음**. Advisor 패널은 `Object.entries(gi)` 루프로 렌더하며 `quiz_poll`만 명시적으로 skip. prompts_advisor.py에서 4필드가 제거되면 LLM이 더 이상 이 키를 리턴하지 않아 루프에서 자동으로 사라짐. 별도 UI 수정 불필요. (spec reviewer 확인)
...
### 위험도
🟢 낮음. 기계적 삭제. 주의점은 두 가지: (1) NewsprintArticleLayout.astro interface 편집 시 `quiz_poll` / `weekly_quiz_expert` / `weekly_quiz_learner` 필드 살아있는지 확인 (라인 기반이 아닌 필드명 기반 삭제), (2) openapi.json의 `$ref` 2곳과 `required` 배열 정리 누락 시 스키마 검증 깨짐.
...
### 영향
- 값은 모두 NULL이라 소실 없음
- WHERE 조건은 post_type 무관하게 4 키 중 하나라도 있는 모든 행에 매칭. 실제로는 이 키들을 쓰던 pre-v4 daily pipeline의 산물뿐이라 daily 포스트만 매칭될 것으로 예상
- 실행 전 `SELECT COUNT(*) FROM news_posts WHERE guide_items ?| array[...]` 로 영향 범위 사전 확인
