...
> [!note] 적용 범위
> 뉴스, 블로그, 핸드북 상세 페이지 모두 공통 적용. 단일 컴포넌트(`ReadingProgressBar`)로 통합.
...
| 768~1023px (태블릿) | 하단 슬림 바 (3px), 액션 바 바로 위 | 본문 넣으면 우측 여백 부족 |
| ~767px (모바일) | 하단 슬림 바 (3px), 액션 바 바로 위 | 동일 |
...
유저가 글 길이를 모르면 스크롤하지 않고 이탈하므로, 모든 콘텐츠 리스트/카드에 "약 N분" 읽기 시간 태그를 표시하여 진입 장벽을 낮춘다.

| 콘텐츠 | 데이터 소스 | 표시 위치 |
| --- | --- | --- |
| 뉴스 | `news_posts.reading_time_min` | 리스트 카드 메타, 상세 헤더 |
| 블로그 | `blog_posts.reading_time_min` | 리스트 아이템 날짜 옆, 피처드 카드, 상세 헤더 |
| 핸드북 | 프론트엔드 계산 (body 길이 기반) | 상세 페이지 헤더 |
