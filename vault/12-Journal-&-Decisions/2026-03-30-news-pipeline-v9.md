...
글쓰기 (gpt-4.1, max_tokens=32K) — 다중 소스 × 전문
    v
후처리: bold fix → tag strip → ★citation renumber → heading aggregate
    v
...
| 항목 | v8 | v9 추가 |
|------|-----|---------|
| 2차 수집 (Exa find_similar) | $0 | ~$0.005 |
| Writer 입력 토큰 증가 (전문 × 다중 소스) | ~$0.02 | ~$0.03 |
| 후처리 (코드) | 0ms | ~5ms |
| **총 run 비용** | **~$0.25** | **~$0.28-0.29** |

비용 12-16% 증가, 하지만 다각도 분석 + citation 정합성 보장.
...
`renumber → aggregate` 순서가 아니면 잘못된 번호가 소제목에 집계됨. 후처리는 데이터 변환 파이프라인이므로 각 단계의 입출력 계약을 명확히 해야 한다.
...
| `90b56bd` | citation per-paragraph + heading 자동 집계 + quality check Source Quality |
