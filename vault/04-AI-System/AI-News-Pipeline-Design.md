...
> 상태: 운영 중 (v5, 2026-03-25~, 진행 중: skeleton-map routing refactor 2026-03-26)
...
- **v3까지**: 팩트 추출 → 3 페르소나 독립 생성
- **v4 (2026-03-17)**: 2 페르소나로 정착 (Beginner 제거 — v4 릴리스 당일)
  - v4 초기에는 3 페르소나가 있었으나, v4 출시 당일(`b145c86`) Beginner 제거됨
  - 그 이후 Expert + Learner만 사용
- **v5 (2026-03-25~)**: 팩트 추출 단계 제거, 2 페르소나 직접 생성 (2+α LLM 호출/카테고리)
  - 팩트 추출 단계가 병목이었고, 모델 성능 향상으로 원문에서 직접 양질의 다이제스트 생성 가능
  - 추가: skeleton-map 기반 라우팅 (post_type별 프롬프트 최적화)
