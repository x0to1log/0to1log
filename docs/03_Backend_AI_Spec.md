...
```sql
-- 글(posts): 누구나 published 읽기 가능, admin_users에 등록된 계정만 쓰기
...
-- 초기 Admin 등록 (운영 시 실제 계정 이메일로 관리)
INSERT INTO admin_users(email) VALUES ('admin@0to1log.com');
```

> **Admin auth contract:** `valid token + admin_users row exists + is_active=true`
...
| 모델 | 필드 | 타입 | 제약 |
|---|---|---|---|
| **QuizPoll** | type | `Literal["quiz","poll"]` | |
| | question | `str` | min_length=10 |
| | options | `list[str]` | 2~4개 |
| | answer | `Optional[str]` | quiz만 해당 |
| **PromptGuideItems** | one_liner | `str` | 10~200자 |
| *(→ posts.guide_items JSONB)* | action_item | `str` | min 20자 |
| | critical_gotcha | `str` | min 20자 |
| | rotating_item_type | `Literal["market_context","analogy","source_check"]` | |
| | rotating_item | `str` | min 20자 |
| | quiz_poll | `QuizPoll` | |
| **ResearchTechSpec** | parameters | `str` | "미공개" 허용 |
| | benchmark | `str` | |
| | improvement | `str` | SOTA 대비 개선율 |
| | exact_quote | `str` | Hallucination 방어용 원문 인용구 |
| **PersonaContent** | content_beginner | `str` | min 100자, 비전공자 버전 |
| | content_learner | `str` | min 100자, 학습자 버전 |
| | content_expert | `str` | min 100자, 현직자 버전 |
| **RelatedNews** | big_tech | `str` | 한 줄 요약 또는 "없음" 메시지 |
| *(→ posts.related_news JSONB)* | industry_biz | `str` | |
| | new_tools | `str` | |
| **RankedCandidate** | title, url | `str` | |
| | assigned_type | `Literal["research","business_main","big_tech","industry_biz","new_tools"]` | |
| | relevance_score | `float` | 0~1 |
| | ranking_reason | `str` | |
| **NewsRankingResult** | batch_id | `str` | |
| | candidates | `list[RankedCandidate]` | |
| | research_pick | `Optional[str]` | Top 1 URL |
| | business_main_pick | `Optional[str]` | Top 1 URL |
| | related_picks | `dict[str, Optional[str]]` | {big_tech, industry_biz, new_tools} |
...
class ResearchPost(BaseModel):
    ...
    category: Literal["ai-news"]
    post_type: Literal["research"]
    has_news: bool
    content_original: Optional[str] = None
    source_urls: list[str] = Field(default=[])
    tech_spec: Optional[ResearchTechSpec] = None
    news_temperature: Optional[int] = Field(default=None, ge=1, le=5)
...
class BusinessPost(BaseModel):
    ...
    slug: str = Field(pattern=r"^[a-z0-9-]+$")
    category: Literal["ai-news"]
    post_type: Literal["business"]
    source_urls: list[str] = Field(min_length=1)
    news_temperature: int = Field(ge=1, le=5)
...
> - `ResearchPost`: has_news=True → content_original 필수 / has_news=False → no_news_notice 필수
> - `BusinessPost`: source_urls의 모든 URL은 "http"로 시작해야 함
>
> **Editorial 판정 기준:**
...
**tech_spec:**
- parameters: 파라미터 수 (미공개면 "미공개")
- benchmark: 주요 벤치마크 점수
- improvement: 기존 SOTA 대비 개선율
...
- **멱등 저장:** `supabase.table("posts").upsert(data, on_conflict="slug")` — 같은 slug면 갱신
- **실패 처리:** `pipeline_logs`에 실패 기록 + `admin_notifications`에 알림 + `pipeline_runs` status='failed'

### 6-8. JSONB 저장 변환

**함수:** `save_post(post, status, batch_id)` — Pydantic 모델 → Supabase posts 테이블

**매핑 규칙:**
- `guide_items` → `PromptGuideItems.model_dump()` → posts.guide_items JSONB
- `related_news` → `RelatedNews.model_dump()` → posts.related_news JSONB (Business만)
...
> 2. `run_daily_news_pipeline()` 1회 실행 → posts 테이블에 Research(status=published) 1행 + Business(status=draft) 1행 적재
...
-- 테스트 4: Admin이 아닌 User는 posts를 INSERT할 수 없는가?
INSERT INTO posts (title, slug, category, post_type) VALUES ('test', 'test', 'ai-news', 'research');
