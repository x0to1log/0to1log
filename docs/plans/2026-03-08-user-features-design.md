...
### 3.4 역할 구분
- `admin_users` 테이블에서 `user_id = auth.uid()`가 일치하면 admin (uid 기반)
...
> **확장 설계:** `docs/plans/2026-03-09-profiles-redesign.md` 참조
...
CREATE TABLE profiles (
  ...
  -- Identity
  display_name          TEXT,
  username              TEXT UNIQUE,        -- 퍼블릭 프로필 URL용 (/u/username)
  avatar_url            TEXT,               -- OAuth 기본값, 유저 업로드 시 덮어쓰기
  bio                   TEXT,               -- 자기소개 (공개 전환 시 표시)
...
| 파일 | 변경 유형 | 영향 |
|------|-----------|------|
| `supabase/migrations/00004_user_tables.sql` | 새 파일 | 없음 (새 테이블 4개) |
...
| 댓글/리액션 | `profiles` 테이블 확장 (interests, social_links 등) |
| 프리미엄 구독 | `user_bookmarks` 패턴으로 `reactions`, `comments` 복제 |

### 확장 시 스키마 변경

- `profiles`에 `interests TEXT[]`, `social_links JSONB` 등 추가 (bio, username, preferred_locale, is_public, onboarding_completed는 이미 추가됨 — `2026-03-09-profiles-redesign.md`)
