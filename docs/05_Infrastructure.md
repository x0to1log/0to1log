...
[GitHub Push: feature/* 브랜치]
      │
      ▼
[Vercel Preview Deployment]
      └── 고유 URL 생성 (https://0to1log-xxx.vercel.app)
          PR 리뷰 시 실제 사이트 확인 가능
```
...
| Preview 브랜치 | `feature/*`, `fix/*` | PR별 프리뷰 환경 자동 생성 |
...
`main`(프로덕션) ← `feature/xxx` 또는 `fix/xxx` 머지. develop 브랜치 없음 (Solo 프로젝트, Preview Deployment가 staging 역할).

- **main 직접 push 금지** (GitHub Branch Protection Rule)
- 모든 변경: PR → CI 통과 → Vercel Preview URL 확인 → 머지
...
| 항목 | 완료 기준 | 실행 주체 |
|---|---|---|
| GitHub 레포 생성 + Branch Protection | main 직접 push 차단, PR 필수 | Amy (수동) |
...
□ GitHub Branch Protection Rule (main 직접 push 차단)
