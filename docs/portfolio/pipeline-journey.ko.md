...
> **기간:** 2026년 2월 중순 – 4월 6일 (기획 2주 + 개발 33일)
...
7개 소스에서 매일 50–60건의 AI 뉴스를 수집하고, 같은 이벤트를 자동 그룹화한 뒤 분류·랭킹·다중 소스 보강·요약하여 2종의 다이제스트(Research + Business)를 Expert/Learner 페르소나로 발행하는 파이프라인. 33일간 10번의 버전을 거쳤고, v10에서 gpt-5로 모델을 전환했다.

| | 시작 (v2) | v8 | v9 + merge | 현재 (v10, gpt-5) |
|---|---|---|---|---|
| **Run당 비용** | $0.18 | $0.25 | $0.43 | **$0.58** |
| **모델** | gpt-4o | gpt-4.1 | gpt-4.1 | **gpt-5** |
| **다이제스트당 출처 인용** | 1.8개 | 16.8개 | 16.8개 | 16.8개 |
| **기사당 참조 소스** | 1개 (원본만) | 1개 | 다중 소스 | 다중 소스 + CP 요약 |
| **수집 소스** | 1개 (Tavily) | 6개 | 7개 | 7개 |
| **품질 점수 (Research)** | 75.8 | 91.8 | 94 | **96** |
| **품질 점수 (Business)** | 82.9 | 94.8 | 95 | **91** |

v2–v8까지 Run당 $0.18–$0.25로 품질을 9.3배 개선. v9에서 비용 폭발($0.77) → merge로 $0.43 복귀. v10에서 gpt-5 전환과 Community Pulse 재설계를 거쳐 $0.58/run, 품질 96/91. 모든 수치는 프로덕션 DB 실측.
...
4. **reasoning 모델은 파라미터 체계가 다르다.** gpt-5에서 빈 응답이 나오면 버그가 아니라 추론 토큰이 출력 예산을 소진한 것. reasoning_effort=low + 3x 헤드룸으로 해결.
...
후처리 (bold fix + 태그 제거 + citation 재번호)
    v
품질 검사 (gpt-5-mini x 4: R/B x Expert/Learner)
    + 코드 감점 (CP 누락 -15, 구조 불일치 -5, 빈 citation -5)
...
| 지표 | | v2–v4 | v5–v6 | v7–v8 | v9 | v10 |
|------|---|-------|-------|-------|-----|-----|
| **품질 점수** | Research | 75.8 | 92.2 | 91.8 | 94 | **96** |
| | Business | 82.9 | 94.1 | 94.8 | 95 | **91** |
| **Expert citation** | Research | 1.8 | 12.9 | 16.8 | 17.5 | 17.5 |
| | Business | 2.7 | 13.9 | 14.2 | 20.5 | 20.5 |
| **Run당 비용** | 전체 | $0.18 | $0.20 | $0.25 | $0.43 | **$0.58** |

*품질 점수는 자동 LLM 평가 (100점 만점). v5부터 4개 페르소나별 평가로 전환하여 기준이 더 엄격해졌음에도 점수가 상승. v10의 Business 91점은 gpt-5 채점 모델 전환에 따른 calibration 차이 — 동일 콘텐츠에서 채점 모델이 바뀌면 절대 점수가 달라진다.*

**요약:** v2–v8까지 $0.18–$0.25로 citation 9.3배 달성. v9에서 $0.77 폭발 → merge로 $0.43 복귀. v10에서 gpt-5 전환 + CP 재설계로 $0.58, 품질 96/91.
...
각 기준 0–25점, 총 100점. gpt-5-mini로 평가 (temperature=0). Expert와 Learner 점수의 평균이 최종 점수.

**레이어 2 — 코드 기반 구조 검증 + 감점**

LLM 채점만으로는 "CP 섹션이 있는가", "EN/KO 섹션 수가 같은가" 같은 구조적 규칙 위반을 잡지 못한다. 코드가 구조를 검증하고 위반 시 감점한다.

최종 점수 = LLM 점수(0–100) - 코드 감점(최대 -30)

CP 데이터 있는데 섹션 누락: -15, EN/KO 섹션 불일치: -5, 빈 citation: -5, Supporting 3문단 미만: -5/item.

Health Check도 코드에서 담당: 분류 0건, merge 과묶기(5+ 아이템), 커뮤니티 수집 0건, enrich 실패. 경고를 기록하되 파이프라인을 차단하지 않는다.
...
|---|---|---|---|---|---|---|---|---|---|---|
| **기간** | 3/10–14 | 3/15 | 3/16 | 3/17 | 3/18–25 | 3/26 | 3/28–29 | 3/29–30 | 3/30 | 3/31–4/6 |
| **결과** | 근본 원인 발견 | 동작 | 동작 | 동작 | 안정화 | 최적화 | 품질 개선 | 구조 분리 | 다중 소스 + merge | gpt-5 + 코드 감점 |
| **모델** | gpt-4o | gpt-4o | gpt-4o | gpt-4o | gpt-4.1 | gpt-4.1 | gpt-4.1 | gpt-4.1 | gpt-4.1 | gpt-5 |
| **비용/run** | N/A | $0.13 | $0.17 | $0.17 | $0.20 | $0.20 | $0.25 | $0.25 | $0.43 | $0.58 |
| **LLM 호출** | 6 | 4 | 6 | 4 | 10 | 10 | 12 | 14 | 14 | 15 |
...
> 10번의 파이프라인 버전, gpt-4o에서 gpt-5까지의 모델 전환,
> 비용 폭발 → merge 복구 → reasoning 모델 마이그레이션을 거쳤고,
> LLM 채점 + 코드 감점 합산이라는 품질 관리 체계를 만들었습니다.
