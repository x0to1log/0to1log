...
| Task ID | 내용 | 의존성 |
|---------|------|--------|
| H2-API-01 | 용어 수집 엔드포인트 (Tavily + OpenAI) | Phase 2B 완료 |
...
- **이중 언어 접미사 방식:** 콘텐츠 필드에 `_en` / `_ko` 접미사 사용. 하나의 용어 = 하나의 행.
  용어(LLM)는 어느 언어에서든 같은 개념이므로 행 분리(posts 방식)보다 적합.
...
**인덱스:** `slug`, `category`, `difficulty`, `status`

**RLS 정책:**
- Public: `status = 'published'` 읽기 허용
- Admin: `admin_users.email = auth.email()` 체크 → 전체 CRUD
...
- **하단 — 관련 기사 (플라이휠 핵심):**
  - posts 테이블에서 tags에 해당 용어 포함된 기사 조회 (최대 5개)
  - 매칭 기사가 없으면 섹션 숨김
...
- Open Graph: term + definition + category
...
- 공통 필드: term, slug, korean_name, difficulty, category
...
**Publish 필수 필드:** `term`, `slug`, `definition_ko`, `category`
...
| 파일 | 변경 유형 | 영향 |
|------|-----------|------|
| `supabase/migrations/00006_*.sql` | 새 파일 | 없음 (새 테이블) |
...
- **데이터:** 별도 테이블 (`handbook_terms` vs `posts`)
- **페이지:** 별도 라우트 (`/handbook/` vs `/log/`)
